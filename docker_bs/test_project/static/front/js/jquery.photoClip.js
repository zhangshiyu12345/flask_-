...
(function(root, factory) {
	...
	function photoClip(container, option) {
		...
		function calculateScale(width, height) {
			...
			myScroll.options.zoomStart = Math.min(myScroll.options.zoomMax, getScale(containerWidth,
		...
		function setTransform($obj, x, y, angle, originX, originY) {
			...
			originY = originY || 0;
			var style = {};
			style[prefix + "transform"] = "translateZ(0) translate(" + x + "px," + y + "px) rotate(" + angle +
				"deg)";
		...
		function setTransition($obj, x, y, angle, dur, fn) {
			...
			$obj.css(prefix + "transform", "translateZ(0) translate(" + x + "px," + y + "px) rotate(" + angle +
				"deg)");
		}
